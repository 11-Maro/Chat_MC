                    ...
                    switch (opcion) {
                        ...
                        while ((message = in.readLine()) != null && !exit) {
                            if (!message.isEmpty()) {
                                ...
                                //     }
                                }if (message.startsWith("/join_call ")) {
                                    String[] parts = message.split(" ");
                                    String chatName = parts[1];
                                    int port = Integer.parseInt(parts[2]);
                                    InetSocketAddress addr = new InetSocketAddress(socket.getInetAddress(), port);
                                    Servidor.joinCall(chatName, addr);
                                } else if (message.startsWith("/leave_call ")) {
                                    String[] parts = message.split(" ");
                                    String chatName = parts[1];
                                    int port = Integer.parseInt(parts[2]);
                                    InetSocketAddress addr = new InetSocketAddress(socket.getInetAddress(), port);
                                    Servidor.leaveCall(chatName, addr);
